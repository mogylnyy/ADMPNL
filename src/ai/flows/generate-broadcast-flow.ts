...
const GenerateBroadcastMessageInputSchema = z.object({
  ...
  targetAudienceDescription: z.string().optional().describe('Опционально: Описание целевой аудитории на основе выбранных категорий (например, "пользователи с премиум подпиской", "новые пользователи в этом месяце", "пользователи, покупавшие товары из категорий: Категория1, Категория2"). ИИ адаптирует сообщение, если предоставлено.'),
});
...
Если в инструкциях пользователя упоминаются конкретные названия товаров, постарайся органично вписать их в сообщение или адаптировать предложение для пользователей, интересующихся этими товарами.
